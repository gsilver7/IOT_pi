...
export class WeatherService {
  ...
  async getVilageFcst(
    ...
    try {
      ...
      if (resultCode !== '00') {
        const errorMsg =
          response.data.response?.header?.resultMsg || 'Unknown API Error';
        throw new InternalServerErrorException(`기상청 API 오류: ${errorMsg}`);
      }

      this.logger.log(
        `Successfully fetched weather data for date: ${baseDate}`,
      );
  ...
  async getNowcast(nxin: string, nyin: string) {
    ...
    try {
      ...
      this.logger.log(
        `Successfully fetched nowcast data for ${base_date} ${base_time}`,
      );
      return this.processNowcastData(rawData);
    } catch (error) {
      if (error instanceof AxiosError) {
        this.logger.error(
          `HTTP Error in getNowcast: ${error.message}`,
          error.stack,
        );
        throw new InternalServerErrorException(
          '외부 API 요청 중 오류가 발생했습니다.',
